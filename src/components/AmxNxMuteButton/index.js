...
const AmxNxMuteButton = (props) => {
    ...
    return (
        <button
            type="button"
            className={`btn btn-square ${btnConfig.style ? btnConfig.style : 'btn-primary'} ${btnConfig.active ? 'active' : ''
        ...
        >
            <div className='d-flex flex-column'>
                {btnConfig.icon && <i className={`${btnConfig.icon} ${btnConfig.textStyle ? btnConfig.textStyle : 'text-light'} fs-2`}></i>}
                <span className={btnConfig.textStyle ? btnConfig.textStyle : 'text-light'} dangerouslySetInnerHTML={{ __html: btnConfig.text }}></span>
            </div>
        </button>
    );
};

export default AmxNxMuteButton;
